...
from altruz.helpers.pictool_help import gib_carbon_sar
from altruz.helpers.pyrogram_help import get_arg
from altruz.core.main_cmd import altruz_on_cmd, e_or_r
from altruz.core.altruz_database.altruz_db_conf import get_custom_var
...
@altruz_on_cmd(command="carbon", modlue=mod_file)
async def gibcarbon(_, message: Message):
    r_msg = message.reply_to_message
    carbon_msg = await e_or_r(altruz_message=message, msg_text="`Executing Command..")
    carbonpic_msg = get_arg(message)
    if not carbonpic_msg:
        if not r_msg:
            await carbon_msg.edit("`Reply to a Text Message`")
            return
        if not r_msg.text:
            await carbon_msg.edit("`Reply to a Text Message`")
            return
    ...
    await ALTRUZ.send_photo(message.chat.id, carboned_pic)
