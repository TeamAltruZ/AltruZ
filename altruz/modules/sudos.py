...
from altruz.core.altruz_database.altruz_db_conf import set_custom_var, get_custom_var
from altruz.core.altruz_database.altruz_db_sudos import add_sudo, remove_sudo, check_if_sudo
from .ub_updater import restart_altruz
...
@altruz_on_cmd(command="addsudo", modlue=mod_file)
async def set_sudo(_, message: Message):
  sudo_msg = await e_or_r(altruz_message=message, msg_text="`Processing...`")
...
@altruz_on_cmd(command="rsudo", modlue=mod_file)
async def set_sudo(_, message: Message):
  sudo_msg = await e_or_r(altruz_message=message, msg_text="`Processing...`")
  sudo_id = str(get_arg(message))
  r_sudo_msg = message.reply_to_message
...
@altruz_on_cmd(command="setvar", modlue=mod_file)
async def setmongovar(_, message: Message):
  setvr_msg = await e_or_r(altruz_message=message, msg_text="`Processing...`")
...
@altruz_on_cmd(command="getvar", modlue=mod_file)
async def get_var(_, message: Message):
  g_var = await e_or_r(altruz_message=message, msg_text="`Processing...`")
