...
# Download path
megadir = "./altruz/Megatools"

# Run bash cmd in python
def altruz_mega_runner(command):
...
def split_files(input_file, out_base_path):
    ...
    altruz_fs.split(file=split_file, split_size=split_fsize, output_dir=out_path)

@altruz_on_cmd(command="megadl", modlue=mod_file)
async def megatoolsdl(_, message: Message):
    megatools_msg = await e_or_r(altruz_message=message, msg_text="`Processing...`")
    url = message.text
    ...
    try:
        for altruz_m in folder_f:
            file_size = os.stat(nexa_m).st_size
            if file_size > 2040108421:
                split_out_dir = altruz_path_f + "splitted_files"
                await megatools_msg.edit("`Large File Detected, Trying to split it!`")
                loop = get_running_loop()
                await loop.run_in_executor(None, partial(split_files(input_file=altruz_m, out_base_path=split_out_dir)))
                await megatools_msg.edit("`Splitting Finished! Uploading Now...`")
                for splitted_f in split_out_dir:
                    await ALTRUZ.send_document(chat_id=message.chat.id, document=splitted_f, caption=f"`Uploaded by` {(await ALTRUZ.get_me()).mention}")
            else:
                chat_id = message.chat.id
                #await guess_and_send(input_file=altruz_m, chat_id=chat_id, thumb_path=altruz_path_f)
                await ALTRUZ.send_document(chat_id=chat_id, document=altruz_m, caption=f"`Uploaded by` {(await.ALTRUZ.get_me()).mention}")
